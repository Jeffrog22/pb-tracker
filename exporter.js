...
const ARN_MEET_NAME = "ARN";

export function buildArnCardRows(event, seriesKey, athlete) {
  const eventName = String(event.eventName).replace(/\s*\|\s*/g, " - ");
  const balizado = formatArnTime(athlete.tempoBalizado);
  const volta = (n) => `*volta ${n}`;
  ...
  return [
    ...
    ["Passagem:", "25m/50m -", volta(1), "", "75m/150m -", volta(3), ""],
    ["", "50m/100m -", volta(2), "", "100m/200m -", volta(4), ""],
    ["Tempo oficial:", "*vazio", "", "Colocação:", "*vazio", "", ""],
  ];
}

function formatArnTime(value) {
  const m = String(value || "").match(/^(\d{2}):(\d{2}):(\d{2})$/);
  if (!m) return String(value || "");
  return `${Number(m[1])}:${m[2]}:${m[3]}`;
}

export async function exportArn({ groupedEvents, getSplitsForEvent, teamName }) {
  const cards = [];
  groupedEvents.forEach((event) => {
    [...event.series.entries()]
      ...
      .forEach(([seriesKey, athletes]) => {
        const sorted = [...athletes].sort((a, b) => Number(a.baliza) - Number(b.baliza));
        sorted.forEach((athlete) => cards.push(buildArnCardRows(event, seriesKey, athlete)));
      });
  });

  if (!cards.length) {
    return { ok: false, reason: "Nenhum atleta importado para exportar ARN." };
  }
  ...
  if (!XLSX) {
    return { ok: false, reason: "Sem internet: é necessário para gerar o XLSX do ARN.", offline: true };
  }

  const rows = [];
  cards.forEach((card, index) => {
    card.forEach((row) => rows.push(row));
    if (index < cards.length - 1) rows.push([]);
  });
  ...
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "ARN");
  const base = `arn-${slugify(teamName)}`;
  const datePart = new Date().toISOString().slice(0, 10).replace(/-/g, "");
  XLSX.writeFile(wb, `${base}-${datePart}.xlsx`);
  return { ok: true, format: "xlsx" };
}
